...
def train(num_mpi_ranks):
    ...
    learning_rate = 1e-03
    log_alpha_data = torch.nn.Parameter(torch.tensor(0.0, dtype=torch.float64, requires_grad=True))
    log_alpha_phys = torch.nn.Parameter(torch.tensor(0.0, dtype=torch.float64, requires_grad=True))
    ...
    while True:
        ...
        for epoch in range(epochs):    
            ...
            # 打印训练信息（可选）
            if epoch % 10000 == 0:
                print(f"Epoch {epoch}, RMSE: {rmse_loss_val:.4e}, Data Loss: {mse_loss.item():.4e}, Phys Loss: {phys_loss.item():.4e}")
