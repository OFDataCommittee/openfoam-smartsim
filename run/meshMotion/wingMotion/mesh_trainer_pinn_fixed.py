...
def train(num_mpi_ranks):
    ...
    while True:    
        ...
        for epoch in range(epochs):    
            ...
            if epoch % 50 == 0:
                data_weight = data_loss.item()/((data_loss.item() + p_loss.item()) + 1e-8)
                physics_weight = p_loss.item()/((data_loss.item() + p_loss.item()) + 1e-8)
                data_weight = 100 * data_weight / (100 * data_weight + physics_weight + 1e-8)
                physics_weight = physics_weight / (100 * data_weight + physics_weight + 1e-8)

            if epoch % 50 == 0 or epoch == epochs - 1:
                print(
                    f"[Epoch {epoch}/{epochs}] "
                    f"data loss: {data_loss.item()}, "
                    f"data weight: {data_weight}, "
